...
class Function
{
	...
	{
		double Sun = -60/(Math.pow(x/2,2)+Math.pow(y/2,2)+10);
		double x1 = 7*Math.sin(t/2);
		double y1 = 7*Math.cos(t/2);
		double d = Math.sqrt(Math.pow(x-x1,2)+Math.pow(y-y1,2));
		double s = -5/(Math.pow(x-x1,2)+Math.pow(y-y1,2)+3);
		if(!(s >=0 || s<0))
			s=0;
		return s+Sun;
	}
...
public class ex1 extends Applet implements MouseListener, MouseMotionListener, Runnable
{	
